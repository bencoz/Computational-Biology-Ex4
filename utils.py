...
def print_model_params(transition, emission, score):
    T_IG = format(transition[0, 1], '.4f')
    T_GI = format(transition[4, 5], '.4f')
    E_IA = format(emission[0]['A'], '.4f')
    E_IT = format(emission[0]['T'], '.4f')
    E_IC = format(emission[0]['C'], '.4f')
    E_GA = format(emission[2]['A'], '.4f')
    E_GT = format(emission[2]['T'], '.4f')
    E_GC = format(emission[2]['C'], '.4f')
